...
# Çalıştırma (lokalde): pip install -r requirements.txt && streamlit run app.py
...
# Twilio importu başarısız olursa uygulama yine çalışsın:
try:
...
# Admin başlangıç bilgileri (Secrets > or Env)
...
def _utc_now_plus(hours: int = 0) -> datetime:
    """UTC uyarısız: datetime.utcnow() yerine timezone-aware kullanalım."""
...
def bootstrap_admin_and_packages():
    with closing(sqlite3.connect(DB_PATH)) as con, con:
        ...
        if cur.fetchone() is None:
            con.execute(
                "INSERT INTO users (username, password_hash, full_name, role, phone, notifications_enabled, created_at) "
                "VALUES (?, ?, ?, ?, ?, ?, ?)",
                (
                    DEFAULT_ADMIN_USER,
        ...
        if cur.fetchone()[0] == 0:
            for name, desc in DEFAULT_PACKAGES:
                con.execute(
                    "INSERT INTO packages (name, description, active) VALUES (?, ?, 1)", (name, desc)
                )
...
def validate_login(username: str, password: str) -> Optional[dict]:
    with closing(sqlite3.connect(DB_PATH)) as con:
        cur = con.execute(
            "SELECT id, username, password_hash, full_name, role, phone, notifications_enabled "
        ...
        if not row:
            return None
        if row[2] != _hash_password(password):
            return None
        return {
            "id": row[0],
            "username": row[1],
...
def list_users() -> pd.DataFrame:
    with closing(sqlite3.connect(DB_PATH)) as con:
        return pd.read_sql_query(
            "SELECT id, username, full_name, role, phone, notifications_enabled FROM users ORDER BY id", con
        )


def list_packages(active_only: bool = True) -> pd.DataFrame:
    with closing(sqlite3.connect(DB_PATH)) as con:
        q = (
            "SELECT id, name, description, active FROM packages WHERE active=1 ORDER BY name"
            if active_only
            else "SELECT id, name, description, active FROM packages ORDER BY name"
        )
        return pd.read_sql_query(q, con)


def upsert_package(
    name: str, description: str, active: bool = True, pkg_id: Optional[int] = None
) -> Tuple[bool, str]:
    with closing(sqlite3.connect(DB_PATH)) as con, con:
        try:
            if pkg_id:
                con.execute(
                    "UPDATE packages SET name=?, description=?, active=? WHERE id=?",
                    (name, description, 1 if active else 0, pkg_id),
                )
                return True, "Paket güncellendi"
            con.execute(
                "INSERT INTO packages (name, description, active) VALUES (?, ?, ?)",
...
def create_patient(row: dict) -> Tuple[bool, str, Optional[int]]:
    try:
        with closing(sqlite3.connect(DB_PATH)) as con, con:
            con.execute(
                """
                INSERT INTO patients (patient_code, full_name, dob, phone, package_id, checkup_date, coordinator,
                                      amount_billed, notes, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
...
def fetch_patients(filters: dict) -> pd.DataFrame:
    query = """
    SELECT p.id, p.patient_code, p.full_name, p.dob, p.phone, pk.name AS package, p.checkup_date,
           p.coordinator, p.amount_billed, p.notes, p.created_at
...
def can_send_whatsapp() -> bool:
    return bool(TWILIO_SID and TWILIO_TOKEN and TWILIO_WHATSAPP_FROM and Client is not None)


def send_whatsapp_message(to_phone_e164: str, body: str) -> bool:
    """to_phone_e164: '+90555XXXXXXX' (sandbox katılımcısı)."""
    if not can_send_whatsapp():
        return False
    try:
        client = Client(TWILIO_SID, TWILIO_TOKEN)
        client.messages.create(from_=TWILIO_WHATSAPP_FROM, to=f"whatsapp:{to_phone_e164}", body=body)
        return True
...
def notify_upcoming_tests() -> int:
    """
    Önümüzdeki 10 dakikada başlayacak ve notified=0 olan testler için
    bildirim gönderir. notifications_enabled=1 kullanıcıları hedefler.
    """
    if not can_send_whatsapp():
        return 0
    now = datetime.now()
    ...
    for t_id, test_name, planned_at, patient_name in rows:
        msg = (
            f"{patient_name} isimli hastamızın {test_name} işlemi 10 dk sonra. "
            f"Lütfen bölümü arayarak teyit alınız ve hastaya eşlik ediniz."
        )
...
if st.session_state.user is None:
    ...
    with colB:
        st.info("İlk giriş: kullanıcı adı **admin**, şifre **Edam456+** (Secrets ile değiştirilebilir).")
    st.stop()
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("WhatsApp için Twilio Sandbox yapılandırması gerekir.")

# Arka planda yaklaşan testler için bildirim taraması (sessiz hata yakala)
try:
...
if page == "Hasta Kayıt":
    ...
    if st.button("Kaydet"):
        if not patient_code or not full_name:
            ...
            (st.success if ok else st.error)(f"{msg}{f' (ID: {pid})' if pid else ''}")
    ...
    df = fetch_patients({"package": pkg_filter or None, "date_range": dr})
    ...
    with c2:
        ...
        desc = st.text_input(
            "Açıklama",
            value=(pkgs[pkgs["name"] == sel]["description"].iloc[0] if sel != "Yeni Paket" and not pkgs.empty else ""),
        )
    with c3:
    ...
    if st.button("Kullanıcı Oluştur"):
        if not new_username or not new_password:
            ...
            ok, msg = create_user(
                new_username.strip(), new_password, new_fullname.strip(), role, new_phone.strip(), notif_on
            )
            (st.success if ok else st.error)(msg)

elif page == "Test Uyarısı (Manuel)":
    st.subheader("Test Uyarısı Gönder (Manuel)")
    to = st.text_input("Kime (örn: +90555xxxxxxx)")
    test_msg = st.text_area(
        "Mesaj",
        value="Örnek: İlyas Ural isimli hastamızın Kardiyoloji muayenesi 10 dk sonra, lütfen teyit alınız ve hastaya eşlik ediniz.",
        height=120,
    )
    if st.button("Mesaj Gönder"):
        if not can_send_whatsapp():
            st.error("Twilio/WhatsApp yapılandırması eksik (Secrets).")
        else:
            ok = send_whatsapp_message(to.strip(), test_msg.strip())
            (st.success if ok else st.warning)("Gönderildi." if ok else "Gönderilemedi.")
